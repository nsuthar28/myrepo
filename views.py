...
def create_checkout_session(request):
    ...
    if request.method == "GET":
        ...
        try:
            ...
            if stripe_customer.trial_end == None: 
                checkout_session = stripe.checkout.Session.create(
                    ...
                    line_items=[
                        {
                            "price": os.environ.get('STRIPE_PRICE_ID','price_1MEWtaSIZVPMggpPVjepjrEm'),
                            "quantity": 1,
                        }
                    ],
                    subscription_data=
...
def handleSubscription(request):
    ...
    stripe.api_key = settings.STRIPE_SECRET_KEY
    stripe_customer = StripeCustomer.objects.get(user=request.user)
    stripe_products = stripe.Product.list(limit=1)
    stripe_product = []
    for product in stripe_products:
        prod = dict()
        prod['trial_status'] = stripe_customer.trial_end
        prod['description'] = product.description
        prod['name'] = product.name
        stripe_plan = stripe.Plan.retrieve(product.default_price)
        prod['price'] = float(stripe_plan.amount/100)
        prod['interval'] = stripe_plan.interval
        if stripe_customer.stripeProductId == product.id:
            prod["status"] = stripe_customer.is_subscribed
        else:
            prod["status"] = False
...
def stripe_webhook(request):
    ...
    if event['type'] == 'checkout.session.completed':
        ...
        # try: 
        #     stripe_subscription_end_final= (stripe_trial_end + timedelta((datetime.datetime.fromtimestamp(stripe_subscription_end) - datetime.datetime.fromtimestamp(stripe_trial_end)).days))
        #     print(stripe_trial_end)
        # except:
        #     ('..............',stripe_subscription_end)
        ...
        StripeCustomer_object.subscription_end = datetime.datetime.fromtimestamp(stripe_subscription_end)
        if stripe_trial_end != None :
            StripeCustomer_object.trial_end = datetime.datetime.fromtimestamp(stripe_trial_end)  
