...
def store_scores(df, fcast_col_name, model_name):
...
def df_w_fcast(df, input_forecast_period):
    ...
    for i in items:
        ...
        df_future = pd.date_range(
            df.index[-1], periods=input_forecast_period+1, freq=data_freq)
        df_future = df_future[1:]
        df_future = pd.DataFrame({'Date': df_future, 'Item': i}).set_index(
            'Date')  # reference column #to loop
...
def lr_rf_fcast_alt(df, df1, df1_f1, model_list, model_title, input_forecast_period):
    ##################
    ### validate it[dhruvi] ###
    items = df['Item'].unique()
    ##################
    ...
    for i in range(len(items)):
        fcast_idx = (df1_f1['Item'] == items[i]) & (
            df1_f1.index > df1.index[-1])  # to loop
        df1_f1.loc[fcast_idx, 'Future_Forecast_{}'.format(
            model_title)] = YYY[i]
...
def run_fcast_full(df, input_forecast_period):
    ...
    dfx, model_aarima = model2_auto_arima(dfx)

    df_results = pd.DataFrame()

    df_results = df_results.append(store_scores(dfx, 'AARIMA', 'ARIMA'))
    # 3rd Model - Machine Learning: LR & RF
    # running model 3 LR and RF
    # df1, lr1, lr2, model_rf = model3_lrrf(df, items)
    df1, lr1_model_list, rf_model_list = model3_lrrf(dfx)

    df1['Qty'] = df1['Qty'].astype(int)

    # Calculating and storing scores
    df_results = df_results.append(
        store_scores(df1, 'LR_multistep', 'LR_Multistep'))
    # df_results = df_results.append(store_scores(df1, 'LR_multioutput','LR_Multioutput'))
    df_results = df_results.append(store_scores(
        df1, 'RF_multistep_test', 'Random Forest'))
    ...
    for i in items:
        df_results_best = df_results_best.append(
            df_results.loc[[df_results.loc[df_results['Item'] == i, 'MAPE'].idxmin()]])
    # Generating new df with fcast dates and empty values

    df1_f1 = df_w_fcast(df1, input_forecast_period)

    # Populating df1_f1 with all the future forecast values for all models

    # running Arima function
    df1_f1 = aarima_fcast(dfx, df1_f1, input_forecast_period)

    # runnning LR function
    df1_f1 = lr_rf_fcast_alt(df, df1, df1_f1, lr1_model_list,
                             'LR', input_forecast_period)

    # running RF function
    df1_f1 = lr_rf_fcast_alt(df, df1, df1_f1, rf_model_list,
                             'RF', input_forecast_period)

    # populating best future forecast as a column based on results
    df1_f1 = populate_best_fcast(df, df1_f1, df_results_best)

    # returning values
    return df_results, df_results_best, df1_f1

# Function to graph all forecasts with loop
def graph_fcast_all(df, df1_f1):
    ##################
    ### validate it[dhruvi] ###
    items = df['Item'].unique()
    ##################
...
def populate_best_fcast(df, df1_f1, df_results_best):
    ##################
    ### validate it[dhruvi] ###
    items = df['Item'].unique()
    ##################
...
def fill_na_json(df, df1_f1, input_forecast_period):
    ##################
    ### validate it[dhruvi] ###
    items = df['Item'].unique()
    ##################
